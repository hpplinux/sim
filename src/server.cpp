...
namespace sim{
/** µÚÒ»¸öÊÇacceptµÄfd£¬µÚ¶þ¸öÊÇ´ý´¦ÀíµÄ¿Í»§¶Ëfd 
Ã²ËÆ²»¶Ô....
**/	
...
/**
Î¨Ò»Ìí¼Ó HANDLER_TYPE ÀàÐÍµÄµØ·½
HANDLER_TYPE µÄÒâË¼ÊÇÌí¼Ó×Ô¶¨ÒåµÄ´¦Àíº¯Êý
ÕâÀïÊÇ×¢²áº¯ÊýµÄµØ·½
*/
void Server::add_handler(Handler *handler){
	...
	if(handler->fd() > 0){
		//°ÑpipeµÄfd¶Ë¿ÚÒ²·ÅÈëepoll½øÐÐ¼à¿Ø
		/** ÕâÀïÊÇ¶ÔinµÄ¼à¿Ø£¬ÄÇÃ´Êý¾ÝÊÇÊ²Ã´Ê±ºòÐ´½øÈ¥µÄÄØ ? */
...
int Server::read_session(Session *sess){
	...
	while(1){
		Request req;
		/**ÎªÊ²Ã´Ò»´Îread, ¶à´ÎrecvÄØ ? 
		ÒòÎªreadÒ»´Î¶Á³öÀ´Õû¸ö»º´æ£¬°ÑÄÜ¶ÁÈ¡µÄÊý¾Ý¶¼¶ÁÈ¡³öÀ´ÁË£¬ºÜ¿ÉÄÜÕâÐ©Êý¾ÝÊÇÓÉ
		¶à¸ö¶ÀÁ¢µÄitem×é³É£¬recvµÄÈÎÎñ¾ÍÊÇÃ¿´Î´Ó»º´æÖÐÈ¡³öÒ»¸öÍêÕûµÄitem½øÐÐÖ´ÐÐ**/
		...
		for(int i=0; i<this->handlers.size(); i++){
			...
			if(state == HANDLE_RESP){//ÊÇ·ñÐèÒª»Ø¸´Õâ¸öreq
				link->send(resp.msg);
...
int Server::loop_once(){
	...
	for(int i=0; i<(int)events->size(); i++){
		...
		if(fde->data.ptr == serv_link){//¼àÌýacceptµÄfd¶Ë¿Ú
			this->accept_session();
